...
`TEAM-SESSION-RESULTS.md`는 이번 세션에서 각 팀이 남긴 결과물 본문을 발췌해 따로 모아 보여준다.
`runtime/team-results/{team}.md`와 `runtime/outputs/session-###/team-results/{team}.md`는 팀별 실제 결과 문서를 별도로 제공한다.
