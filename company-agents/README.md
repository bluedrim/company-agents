...
다음 세션은 직전 세션의 팀별 산출물과 CEO 세션 리뷰를 읽고 필요한 것은 발전시키며, 더 이상 필요하지 않은 산출물은 사용하지 않아도 된다는 지시를 agent prompt에 포함한다.
각 실행은 `runtime/DASHBOARD.md`, `runtime/CYCLE-BRIEF.md`, `runtime/OPERATING-REVIEW.md`, `runtime/CEO-SESSION-REVIEW.md`를 함께 만들어 운영자가 팀별 업무량, 실패 agent, 승인 필요사항, CEO 평가, 다음 세션 지시를 빠르게 확인할 수 있게 한다.
